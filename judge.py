    ...
    if result_message["CE"] in log:
        ...
        for i in range(3, len(logs) - 1):
            ...
            if logs[i].split(".")[0] == problem_name:
                ...
                logs[i].split(" ")[0] == "Error:" and language == "pas"
            ):  # Pascal, why?
                continue
        ...
        total_points = round(
            float(logs[0].split(" ")[-1].replace(",", ".")), round_digits
        )
        ...
        while i < len(logs):
            ...
            points = round(
                float(logs[i].split(" ")[-1].replace(",", ".")), round_digits
            )
            execution_time = 0
            ...
            for k in range(i, j + 1):
                ...
                for message_code in ["WA", "RE", "TLE", "NF"]:
                    if (
                        result_message[message_code] in logs[k]
                        and message == result_message["AC"]
                    ):
...
while True:
    ...
    submission_name = (
        f"{str(current_row - 1).zfill(4)}[{contestant}][{problem_name}].{extension}"
    )

    if status == "":
        send_request(
            sheet.update,
            [["Đang chờ", judge_id, "Themis"]],
        ...
        if status == "Đang chờ":
            ...
            if os.path.exists("Submissions/Logs/" + submission_name + ".log"):
                ...
                with open(
                    "Submissions/Logs/" + submission_name + ".log", "r", encoding="utf8"
                ) as f:
                    (
                        total_points,
                        max_execution_time,
                        ...
                        failed_test,
                        tests_result,
                    ) = log_parser(f.read(), problems_data[problem_id])
                    formatted_log = format_log(
                        total_points, max_execution_time, final_message, tests_result
                    )
